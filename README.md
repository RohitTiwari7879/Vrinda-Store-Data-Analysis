...
1.  **Sales vs. Orders:** Monthly trend analysis to compare total sales amount against the total count of order IDs.
2.  **Sales: Men vs. Women:** A pie chart illustrating the revenue split by gender.
3.  **Sales: Top 5 States:** A bar chart showing the five states with the highest sales volume.
4.  **Order: Age vs. Gender:** A clustered column chart displaying order patterns by both age group and gender.
5.  **Order Status:** A chart summarizing the breakdown of orders by status (e.g., Delivered, Refunded, Cancelled).
6.  **Orders: Channels:** A pie chart showing the distribution of orders across different sales channels like Amazon, Flipkart, etc.
